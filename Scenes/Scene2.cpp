...
Scene* Scene2::Build(Application* app)
{
    ...
    // Point light
    PointLight* point = new PointLight(lightCenter + glm::vec3(0.f, 0.7f, 0.f),glm::vec3(0.9f, 0.7f, 0.7f),0.25f);
    point->attach(sh2);
    point->notify();
    scene->addLight(point);

    // Directional
    DirectionalLight* directional = new DirectionalLight(glm::normalize(glm::vec3(0.5f, -1.f, 0.3f)),glm::vec3(0.9f, 0.9f, 1.f),0.03f);
